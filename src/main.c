/*
 * rsa.h
 *
 *  Created on: 02-Oct-2015
 *      Author: Thorsten Wilmer
 */

/*

  valgrind --tool=drd --show-stack-usage=yes ./rsa-test3
 ==26046== thread 1 finished and used 1119464 bytes out of 8388608 on its stack. Margin: 7269144 bytes.
==26098== thread 1 finished and used 1118424 bytes out of 8388608 on its stack. Margin: 7270184 bytes.
==26384== thread 1 finished and used 1110152 bytes out of 8388608 on its stack. Margin: 7278456 bytes.

==27467== thread 1 finished and used 189064 bytes out of 8388608 on its stack. Margin: 8199544 bytes.
==27571== thread 1 finished and used 123272 bytes out of 8388608 on its stack. Margin: 8265336 bytes.
==27913== thread 1 finished and used 73896 bytes out of 8388608 on its stack. Margin: 8314712 bytes.
==32466== thread 1 finished and used 69816 bytes out of 8388608 on its stack. Margin: 8318792 bytes
==1942==  thread 1 finished and used 42632 bytes out of 8388608 on its stack. Margin: 8345976 bytes.
// after some bug fixes
==2711== thread 1 finished and used 46808 bytes out of 8388608 on its stack. Margin: 8341800 bytes.
// reduction of buffer sizes to minimum
==13331== thread 1 finished and used 26504 bytes out of 8388608 on its stack. Margin: 8362104 bytes.

 */

#include "rsa.h"
#include <stdio.h>
#include <stdint.h>

void mp_print( char* pref,  mp_int *v)
{
	char text[200];
	mp_toradix_n(v, text, 16, 200);
	text[199]=0;
	printf("%s %s\n", pref, text);

	return;
	int len = mp_unsigned_bin_size(v);
	uint8_t data[len];
	int res= mp_to_unsigned_bin (v, data);
	printf("Res %d\n", res);
	printf("uint32_t %s_len=%d;",pref, len);
	printf("uint8_t %s[%d]={", pref,len);
	for(int i=0;i<len;i++)
	{
		printf("0x%02x, ", data[i]);

	}
	printf("};\n");

}

/*int mp_unsigned_bin_size(mp_int *a);
int mp_read_unsigned_bin(mp_int *a, const unsigned char *b, int c);
int mp_to_unsigned_bin(mp_int *a, unsigned char *b);
int mp_to_unsigned_bin_n (mp_int * a, unsigned char *b, unsigned long *outlen);
*/
uint32_t N_len=512;
	uint8_t N[512]={, };

	uint32_t result_len=512;
	uint8_t result[512]={, };

int test1()
{
	printf("Test1\n");


		mp_int d;
	mp_int n;
	mp_int res;
	mp_int output;
	mp_init(&d);
	mp_init(&n);
	mp_init(&output);
	mp_init(&res);
	mp_set(&d, 5);
	if(0!=mp_read_unsigned_bin(&res,result, result_len))
	{
		printf("Error reading result\n");
	}
	mp_print("Res",&res);

	mp_read_unsigned_bin(&n,N, N_len);
	mp_print("N",&n);
	EncryptDecrypt(&output,&res, &d, &n);
	mp_print("Output",&output);

	return 0;
}

int main()
{
	//test1();


	//return 0;


	mp_int d;
	mp_int e;
	mp_int n;
	mp_int msg;
	mp_int result;
	mp_int output;

	mp_init(&d);
	mp_init(&e);
	mp_init(&n);
	mp_init(&msg);
	mp_init(&result);
	mp_init(&output);

	GenerateKeys(&d, &e, &n);

	mp_set_int(&msg, 0x12345678);
	mp_print("Input",&msg);
	mp_print("E",&e);
	mp_print("D",&d);
	mp_print("N",&n);
	EncryptDecrypt(&result, &msg, &e, &n);
	mp_print("result",&result);
    EncryptDecrypt(&output,&result, &d, &n);

	mp_print("Output",&output);
}
